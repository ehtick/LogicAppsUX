...
## Corpus Check
- 567 files · ~195,806 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1251 nodes · 985 edges · 34 communities detected
...
- `serializeAuthentication()` --calls--> `convertStringToSegments()`  [INFERRED]
  src/lib/authentication/util.ts → src/lib/editor/base/utils/editorToSegment.ts
- `tokenClicked()` --calls--> `buildInlineCodeTextFromToken()`  [INFERRED]
  src/lib/code/index.tsx → src/lib/code/util.ts

## Communities

### Community 0 - "Community 0"
Cohesion: 0.05
Nodes (34): collapseLiteralSegments(), convertStringToSegments(), removeNewlinesAndSpaces(), isTokenValueSegment(), handleToggle(), handleValueChange(), toggleCollapsed(), updateComplexItems() (+26 more)

### Community 1 - "Community 1"
Cohesion: 0.06
Nodes (21): handleBlur(), onFileFolderSelected(), GroupDropdown(), addStringLiteralSegment(), containsTokenSegments(), createLiteralValueSegment(), insertQutationForStringType(), handleClearClick() (+13 more)
...
Cohesion: 0.07
Nodes (21): onChange(), handleOptionSelect(), Format(), handleCollapsedBlur(), renderAuthentication(), serializeCodeCollapsedValue(), updateItems(), onTitleBlur() (+13 more)

### Community 4 - "Community 4"
Cohesion: 0.08
Nodes (22): handleArrayElementSaved(), handleCheckbox(), handleDelete(), handleGroup(), handleMoveChild(), handleSelectedOption(), handleUngroup(), handleUpdateNewParent() (+14 more)

### Community 5 - "Community 5"
Cohesion: 0.07
Nodes (14): handleBlur(), getLabel(), handleBlur(), handleClick(), handleClickOrEnterKeyPress(), handleContentChanged(), handleEnterKeyPress(), handleFileNameChange() (+6 more)
...
Cohesion: 0.18
Nodes (12): capitalizeElements(), castParameterValueToPrimitiveType(), convertComplexItemsToArray(), convertObjectToComplexArrayItemArray(), getOneDimensionalSchema(), handleArrayOfComplexItems(), handleSimpleItem(), handleTitle() (+4 more)
...
Cohesion: 0.2
Nodes (10): SchemaEditor(), buildInlineCodeTextFromToken(), formatForCSharp(), formatForJavascript(), formatForPowershell(), formatForPython(), getEditorHeight(), getInitialValue() (+2 more)
...
Cohesion: 0.17
Nodes (3): saveStaticResults(), formatShownProperties(), serializePropertyValues()
...
- **Thin community `Community 31`** (5 nodes): `getLanguageExtension()`, `languageHasBuiltInCompletion()`, `workflow()`, `CodeMirrorEditor.tsx`, `index.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 32`** (5 nodes): `getCreateNewFolderToken()`, `getInitializeVariableAbcToken()`, `getInitializeVariableOpenBraceToken()`, `getOneDriveFileContentToken()`, `editorToSegment.spec.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 33`** (5 nodes): `SecureDataSection()`, `utils.ts`, `index.tsx`, `getBrandColorWithOpacity()`, `getHeaderStyle()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (4 nodes): `Assertions()`, `assertions.tsx`, `theme.ts`, `isHighContrastBlack()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (3 nodes): `buildFoundryPortalUrl()`, `guidToBase64Url()`, `index.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (3 nodes): `InvalidJsonSchemaTypeException`, `.constructor()`, `invalidjsonschematype.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 62`** (3 nodes): `DecimalValue()`, `localizeDecimalNumber()`, `decimal.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `createLiteralValueSegment()` connect `Community 1` to `Community 0`, `Community 5`, `Community 10`, `Community 12`, `Community 16`, `Community 17`?**
  _High betweenness centrality (0.047) - this node is a cross-community bridge._
- **Why does `convertStringToSegments()` connect `Community 0` to `Community 8`, `Community 1`, `Community 11`, `Community 4`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `onChange()` connect `Community 3` to `Community 0`, `Community 1`, `Community 2`, `Community 4`, `Community 5`, `Community 12`, `Community 14`, `Community 17`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Are the 30 inferred relationships involving `onChange()` (e.g. with `updateItems()` and `handleCollapsedBlur()`) actually correct?**
  _`onChange()` has 30 INFERRED edges - model-reasoned connections that need verification._
- **Are the 19 inferred relationships involving `createLiteralValueSegment()` (e.g. with `convertKeyValueItemToSegments()` and `convertStringToSegments()`) actually correct?**
  _`createLiteralValueSegment()` has 19 INFERRED edges - model-reasoned connections that need verification._
- **Are the 14 inferred relationships involving `convertStringToSegments()` (e.g. with `getParameterValue()` and `createVariableEditorSegments()`) actually correct?**
  _`convertStringToSegments()` has 14 INFERRED edges - model-reasoned connections that need verification._
- **Are the 12 inferred relationships involving `convertSegmentsToString()` (e.g. with `parseVariableEditorSegments()` and `parseSchemaAsVariableEditorSegments()`) actually correct?**
